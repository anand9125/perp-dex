...
import { connection, program, getLiquidatorKeypair, globalConfigPda, eventQueuePda, marketPda, bidsPda, asksPda, positionPdaFromSymbol, userCollateralPda, getAllMarkets, PROGRAM_ID, idl, } from './shared.js';
...
function accountHealthSingle(collateral, basePosition, entryPrice, markPrice, mmBps) {
...
async function main() {
    ...
    const vaultQuote = globalConfig.vaultQuote;
    const insuranceFund = globalConfig.insuranceFund;
    ...
    for (;;) {
        try {
            ...
            for (const { pubkey: positionPk, account } of positions) {
                ...
                const health = accountHealthSingle(collateral, basePosition, entryPrice, markPrice, mmBps);
                if (health >= BigInt(0))
                    continue;
                ...
                try {
                    ...
                    if (e.message?.includes('NothingToLiquidate') || e.logs?.some((l) => l.includes('NothingToLiquidate'))) {
