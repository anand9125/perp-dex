...
export function getLiquidatorKeypair() {
...
export const requestQueuePda = PublicKey.findProgramAddressSync([Buffer.from('request_queue')], PROGRAM_ID)[0];
export const eventQueuePda = PublicKey.findProgramAddressSync([Buffer.from('event_queue')], PROGRAM_ID)[0];
export const globalConfigPda = PublicKey.findProgramAddressSync([Buffer.from('global_config')], PROGRAM_ID)[0];
export async function getRequestQueueCount() {
    const info = await connection.getAccountInfo(requestQueuePda);
    if (!info?.data || info.data.length < QUEUE_COUNT_OFFSET + 2)
        return 0;
 ...
 */
export function peekEventQueueHeadUser(data) {
    ...
    if (len < MATCHED_ORDER_USER_OFFSET + MATCHED_ORDER_USER_LEN)
        return null;
...
export function marketPda(symbol) {
    return PublicKey.findProgramAddressSync([Buffer.from('market'), Buffer.from(symbol)], PROGRAM_ID)[0];
}
export function bidsPda(symbol) {
    return PublicKey.findProgramAddressSync([Buffer.from('bids'), Buffer.from(symbol)], PROGRAM_ID)[0];
}
export function asksPda(symbol) {
    return PublicKey.findProgramAddressSync([Buffer.from('asks'), Buffer.from(symbol)], PROGRAM_ID)[0];
}
export function positionPdaFromSymbol(symbol, userPk) {
    return PublicKey.findProgramAddressSync([Buffer.from('position'), Buffer.from(symbol), userPk.toBuffer()], PROGRAM_ID)[0];
}
export function userCollateralPda(userPk) {
    return PublicKey.findProgramAddressSync([Buffer.from('user_colletral'), userPk.toBuffer()], PROGRAM_ID)[0];
}
